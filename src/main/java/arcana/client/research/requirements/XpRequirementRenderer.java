...
public class XpRequirementRenderer implements RequirementRenderer<XpRequirement>{
	...
	public void render(DrawContext ctx, int x, int y, XpRequirement requirement, int time, float delta){
		doXPRender(ctx, time, x, y, delta);
	}
	...
	public static void doXPRender(DrawContext ctx, int time, int x, int y, float delta){
		int u = 0, v = 16;
		...
		RenderHelper.drawTexture(ctx, x, y, 100, u, v, 16, 16, 64, 64, red, 1, blue, 1);
		ctx.getMatrices().pop();
	}
}