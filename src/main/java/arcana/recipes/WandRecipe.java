...
public class WandRecipe extends SpecialCraftingRecipe{
	...
	public boolean matches(CraftingInventory inventory, World world){
		...
		if(inventory.getStack(8).getItem() != caps)
			return false;
		// core in middle
		if(asCore(inventory.getStack(4)) == null)
			return false;
		// nothing else
		for(int i = 0; i < 9; i++)
			if(i != 0 && i != 4 && i != 8)
	...
	public ItemStack craft(CraftingInventory inventory){
		...
		Core core = asCore(inventory.getStack(4));
	...
	private static Core asCore(ItemStack is){
		return is.getItem() instanceof Core core ? core : is.getItem() == Items.STICK ? ArcanaRegistry.STICK_CORE : null;
	}
}