...
public class CrystallizationPressScreen extends HandledScreen<CrystallizationPressScreen.Handler>{
	...
	protected void drawBackground(DrawContext ctx, float delta, int mouseX, int mouseY){
		...
		if(handler.getQuartzAmount() > 0){
			...
			ctx.drawTexture(texture, x + 44, y + 44 + (18 - pixels), 176, 18 - pixels, 4, pixels);
		}
		...
		if(handler.getProgress() > 0){
			...
			ctx.drawTexture(texture, x + 97, y + 33, 180, 0, pixels, 13);
		}
		...
		if(handler.getEssentiaAmount() > 0){
			...
			ctx.drawTexture(texture, x + 67, y + 13 + (52 - pixels), 202, 52 - pixels, 16, pixels);
		}
	}
